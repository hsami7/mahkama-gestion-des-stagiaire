...
export function InternPortal() {
  ...
  const pendingCount = useMemo(() => {
    ...
    return reqs + signFill + awaitingIntern + revisionReqs + missingTemplateDocs + unreadNotifs;
  }, [requests, lifecycleDocs, notifications]);
                              ...
                              return rows.map(row => {
                                if (row.isCombined) {
                                  ...
                                  if (d.status === 'APPROVED_AND_SIGNED' && fillDoc.status === 'APPROVED_AND_SIGNED') {
                                    ...
                                    statusClass = 'badge-warning';
                                  } else if (d.status === 'AWAITING_ADMIN' || fillDoc.status === 'AWAITING_ADMIN') {
                                    statusLabel = 'بانتظار الإدارة';
                                    statusClass = 'badge-warning';
                                  } else if (d.status === 'REVISION_REQUESTED' || fillDoc.status === 'REVISION_REQUESTED') {
                                    statusLabel = 'مطلوب إعادة';
                                    ...
                                    <tr key={row.id} style={{ borderBottom: '1px solid var(--line)' }}>
                                      <td style={{ padding: '10px 8px' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, width: '100%', minWidth: 0 }}>
                                          <span style={{ fontWeight: 600, color: 'var(--ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: '0 1 auto', minWidth: 0 }} title={row.base}>
                                      {truncateWords(row.base, 5)}
                                    </span>
                                          <span style={{ fontSize: 10, color: 'var(--slate-light)', flexShrink: 0, whiteSpace: 'nowrap' }}>
                                            {row.isSignFill ? '(توقيع وتعبئة)' : '(توقيع) و (تعبئة وإرجاع)'}
                                          </span>
                                        </div>
                                      ...
                                      <td style={{ textAlign: 'left', padding: '10px 8px' }}>
                                        <div style={{ display: 'flex', gap: 4, flexDirection: 'row-reverse', justifyContent: 'flex-start', flexWrap: 'nowrap' }}>
                                          ...
                                          {(d.status === 'AWAITING_RETURN' || fillDoc.status === 'AWAITING_RETURN') && d.requested_by === 'ADMIN' && (
                                            <button className="btn btn-ghost sm" title="طلب إعادة رفع" onClick={async () => {
                                              ...
                                              } catch (err: any) { showToast(err.response?.data?.msg || 'فشل إرسال الطلب', 'error'); }
                                          ...
                                          {(d.status === 'AWAITING_ADMIN' || fillDoc.status === 'AWAITING_ADMIN') && d.requested_by === 'INTERN' && (
                                            <button className="btn btn-ghost sm" title="حذف الطلب" onClick={async () => {
                                              ...
                                              try {
                                                ...
                                                fetchLifecycleDocs();
                                              } catch (err: any) { showToast(err.response?.data?.msg || 'فشل حذف الطلب', 'error'); }
                                          ...
                                          {(((d.status === 'REVISION_REQUESTED' && d.revision_requested_by !== 'INTERN') || (fillDoc.status === 'REVISION_REQUESTED' && fillDoc.revision_requested_by !== 'INTERN')) || (d.status !== 'APPROVED_AND_SIGNED' && fillDoc.status !== 'APPROVED_AND_SIGNED' && d.status !== 'AWAITING_ADMIN' && fillDoc.status !== 'AWAITING_ADMIN' && d.status !== 'AWAITING_INTERN' && fillDoc.status !== 'AWAITING_INTERN' && (d.status !== 'REVISION_REQUESTED' || d.revision_requested_by !== 'INTERN') && (fillDoc.status !== 'REVISION_REQUESTED' || fillDoc.revision_requested_by !== 'INTERN') && !bothReturned)) && (
                                            <>
                                              ...
                                              <button className="btn btn-ink sm" style={{ padding: '4px 8px', fontSize: 11 }} onClick={() => document.getElementById(inputId)?.click()}>
                                                <UploadSimple size={14} /> {(d.status === 'REVISION_REQUESTED' || fillDoc.status === 'REVISION_REQUESTED') ? 'إعادة رفع' : 'رفع'}
                                              </button>
                                            </>
                                          )}
                                          {/* Greyed upload if already returned but not yet approved */}
                                          {d.status !== 'APPROVED_AND_SIGNED' && fillDoc.status !== 'APPROVED_AND_SIGNED' && d.status !== 'AWAITING_RETURN' && fillDoc.status !== 'AWAITING_RETURN' && bothReturned && d.requested_by !== 'INTERN' && d.status !== 'REVISION_REQUESTED' && fillDoc.status !== 'REVISION_REQUESTED' && (
                                          ...
                                          {/* AWAITING_INTERN — admin uploaded, intern can accept or request revision */}
                                          {(d.status === 'AWAITING_INTERN' || fillDoc.status === 'AWAITING_INTERN') && d.returned_file_path && (
                                            <>
                                              <button className="btn btn-ghost sm" title="قبول" onClick={async () => {
                                                ...
                                                } catch (err: any) { showToast(err.response?.data?.msg || 'فشل قبول المستند', 'error'); }
                                ...
                                const getStatusLabel = () => {
                                  ...
                                  if (d.status === 'REVISION_REQUESTED') return d.revision_requested_by === 'INTERN' ? 'بانتظار تعديل الإدارة' : 'مطلوب إعادة من المتدرب';
                                ...
                                const canUpload = (d.status === 'REVISION_REQUESTED' && d.revision_requested_by !== 'INTERN') || (d.status !== 'APPROVED_AND_SIGNED' && d.status !== 'AWAITING_ADMIN' && d.status !== 'AWAITING_INTERN' && d.status !== 'RETURNED' && d.status !== 'REVISION_REQUESTED' && (!isInternInitiatedSignFill || !d.file_path) && !isView);
                                const isUploadDisabled = isTemplatePendingIntern;
                                return (
                                  <tr key={d.id} style={{ borderBottom: '1px solid var(--line)' }}>
                                    <td style={{ padding: '10px 8px' }}>
                                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, width: '100%', minWidth: 0 }}>
                                        <span style={{ fontWeight: 600, color: 'var(--ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: '0 1 auto', minWidth: 0 }} title={fileLabel}>
                                          {truncateWords(fileLabel, 5)}
                                        </span>
                                        {(() => {
                                          const isTemplate = d.source === 'TEMPLATE_VIEW';
                                          if (isTemplate) return <span style={{ fontSize: 10, color: 'var(--slate-light)', flexShrink: 0, whiteSpace: 'nowrap' }}>(مستند مطلوب)</span>;
                                          const badgeMap: any = {
                                            'view': 'عرض فقط',
                                            'sign': 'توقيع',
                                            'fill': 'تعبئة وإرجاع',
                                            'sign_fill': 'توقيع وتعبئة',
                                            'upload': 'طلب رفع مستند',
                                          };
          ...
          <div className="modal" style={{ maxWidth: 600 }}>
            ...
            <div className="form-group" style={{marginTop:16}}>
                ...
                <div style={{display:'flex', flexDirection:'column', gap:8, padding:'8px 12px', background:'var(--paper)', borderRadius:8, border:'1px solid var(--line)'}}>
                  {(['view','sign','fill','upload'] as const).map(type => {
                    ...
                    <label key={type} style={{display:'flex', alignItems:'center', gap:10, cursor: viewLocked ? 'not-allowed' : 'pointer', fontSize:13, padding:'4px 0', opacity: viewLocked ? 0.5 : 1}}>
                      ...
                      <span style={{fontWeight: internUploadActionTypes.has(type) ? 600 : 400}}>
                        ...
                        {type === 'upload' ? 'طلب رفع مستند — طلب رفع مستند من الإدارة (بدون قالب)' : ''}
                      </span>
                    </label>
                    );
                  })}
            ...
            <div className="modal-foot">
              ...
              {Array.from(internUploadActionTypes).some(t => t === 'sign' || t === 'fill') && !internUploadFile && (
                <div style={{color:'#DC2626', fontSize:12, marginBottom:8}}>يجب رفع ملف لطلبات التوقيع أو التعبئة</div>
              )}
              <button className="btn btn-gold" disabled={
                ...
                let primaryType = types[0] || 'view';
                if (types.includes('sign') && types.includes('fill')) primaryType = 'sign_fill';
                try {
