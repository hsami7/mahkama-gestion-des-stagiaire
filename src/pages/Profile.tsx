...
export function Profile() {
  ...
  if (!isAdmin && user?.permissions) {
    try {
      ...
      if (perms?.doc_reupload?.delete) canDeleteDocs = true;
    } catch (e) { }
  }
  ...
  const saveEncadrant = async () => {
    try {
      ...
      toast.error(err.response?.data?.msg || "فشل في حفظ المؤطر");
    }
  };
  ...
  const markAttendance = async (status: string) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.msg || "فشل في تسجيل الحضور");
    }
  };
  ...
  const handleDelete = async () => {
    if (window.confirm('هل أنت متأكد من حذف هذا المتدرب؟')) {
      try {
        ...
        console.error(err);
        toast.error(err.response?.data?.msg || 'فشل الحذف');
      }
    }
  };
  ...
  const confirmApprove = async () => {
    ...
    try {
      ...
      toast.success('تم قبول المتدرب وتنشيط حسابه بنجاح!');
        ...
        toast.error(err.response?.data?.msg || 'فشل القبول');
    }
  };
  ...
  const saveEvaluation = async () => {
    ...
    try {
      ...
      setShowEvalForm(false);
        ...
        toast.error(err.response?.data?.msg || 'فشل حفظ التقييم');
    } finally {
      setSavingEval(false);
    }
  };
  ...
  const handleUploadSigned = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } catch (err: any) { toast.error(err.response?.data?.msg || 'فشل رفع النسخة الموقعة'); }
  ...
  const handleReject = async () => {
    if (window.confirm('هل أنت متأكد من رفض هذا المتدرب؟')) {
      try {
        ...
        toast.error(err.response?.data?.msg || 'فشل الرفض');
      }
    }
  };
  ...
  return (
    <div>
      ...
      <div className="grid-2">
                              ...
                              <tr key={row.id} style={{ borderBottom: '1px solid var(--line)' }}>
                                ...
                                <td style={{ textAlign: 'left', padding: '10px 8px' }}>
                                  <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                                        ...
                                        <button className="btn btn-ghost sm" onClick={() => {
                                          ...
                                          input.onchange = async () => {
                                            ...
                                            try {
                                              ...
                                              fetchDocsLifecycle();
                                            } catch (err: any) { toast.error(err.response?.data?.msg || 'فشل رفع النسخة'); }
                                          };
                                          input.click();
                                    ...
                                    {canManageDocs && d.status === 'REVISION_REQUESTED' && d.revision_requested_by === 'INTERN' && (
                                      <button className="btn btn-ghost sm" onClick={() => {
                                        const input = document.createElement('input');
                                        input.type = 'file';
                                        input.accept = '.pdf,.doc,.docx,.png,.jpg,.jpeg';
                                    ...
                                    {canDeleteDocs && d.status === 'AWAITING_INTERN' && anyReturned && (
                                      <button className="btn btn-ghost sm" onClick={async () => {
                                        ...
                                        try {
                                          ...
                                          fetchDocsLifecycle();
                                        } catch (err: any) { toast.error(err.response?.data?.msg || 'فشل حذف النسخة'); }
                                    ...
                                    {canDeleteDocs && row.sign.requested_by === 'ADMIN' && d.status !== 'AWAITING_INTERN' && fillDoc.status !== 'AWAITING_INTERN' && (
                                      <button className="btn btn-ghost sm" onClick={async () => {
        ...
        toast.error(err.response?.data?.msg || 'فشل حذف المستندات');
                                        }
                          ...
                          const actionLabelMap: any = { 'view': 'عرض فقط', 'sign': 'توقيع', 'fill': 'تعبئة النموذج', 'sign_fill': 'توقيع وتعبئة', 'upload': 'طلب رفع مستند' };
                          ...
                          const showApprove = canManageDocs && d.status === 'PENDING_REVIEW' && !isTemplateAdminPending && d.uploaded_by !== 'ADMIN';
                            ...
                            <tr key={d.id} style={{ borderBottom: '1px solid var(--line)' }}>
                              <td style={{ padding: '10px 8px' }}>
                                <div style={{ fontWeight: 600, color: 'var(--ink)', maxWidth: 280, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} title={d.custom_title || d.doc_type}>
                                  <bdi>
                                    {(() => {
                                      const t = (d.custom_title || d.doc_type || '').replace(/_/g, ' ');
                                      const words = t.split(' ');
                                    ...
                                    <>
                                      ...
                                      {d.status === 'PENDING_REVIEW' && (
                                        <>
                                          ...
                                          {(d.action_type === 'view' || d.action_type === 'upload') && d.file_path ? (
                                            <button className="btn btn-ghost sm" onClick={() => {
                                          ...
                                          {canDeleteDocs && (
                                            <button className="btn btn-ghost sm" onClick={async () => {
                                              ...
                                              try {
                                                ...
                                                toast.error(err.response?.data?.msg || 'فشل حذف النسخة');
                                              }
                                            }} title="حذف الرفع" style={{ width: 28, height: 28, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--danger)' }}>
                                              <Trash size={14} />
                                            </button>
                                      ...
                                      {d.status === 'REVISION_REQUESTED' && (
                                        <>
                                          ...
                                          {d.revision_requested_by === 'INTERN' && canManageDocs && (
                                            <button className="btn btn-ghost sm" onClick={() => {
                                              const input = document.createElement('input');
                                              input.type = 'file';
                                              input.accept = '.pdf,.doc,.docx,.png,.jpg,.jpeg';
                                      ...
                                      {canDeleteDocs && d.requested_by === 'ADMIN' && (
                                            <button className="btn btn-ghost sm" onClick={async () => {
        ...
        toast.error(err.response?.data?.msg || 'فشل حذف المستند');
                                              }
                                            }} title="حذف" style={{ width: 28, height: 28, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--danger)' }}>
                                              <Trash size={14} />
                                            </button>
                                    ...
                                    <>
                                      ...
                                      {canManageDocs && d.action_type !== 'view' && ((d.status === 'REVISION_REQUESTED' && d.revision_requested_by === 'INTERN') || d.status === 'AWAITING_ADMIN' || d.status === 'RETURNED') && (
                                        <button className="btn btn-ghost sm" onClick={() => {
                                          const input = document.createElement('input');
                                          input.type = 'file';
                                          input.accept = '.pdf,.doc,.docx,.png,.jpg,.jpeg';
        ...
        toast.error(err.response?.data?.msg || 'فشل حذف المستند');
          ...
          <div style={{ borderTop: '1px solid var(--line)', paddingTop: 16 }}>
            <button className="btn btn-ghost" style={{ width: '100%', justifyContent: 'center', padding: '10px', fontSize: 12.5, border: '1.5px dashed var(--line)', borderRadius: 8, color: 'var(--slate)' }} onClick={async () => {
              if (!id) return;
              try {
                const res = await fetch(`${API_BASE}/interns/${id}/export-zip?token=${localStorage.getItem('token')}`);
      ...
      {showAssignModal && (
        ...
        toast.error(err.response?.data?.msg || 'فشل رفع الوثيقة الموقعة');
                }
              }}>
                رفع الوثيقة الموقعة
              </button>
      ...
      {showRevisionModal && (
        ...
        toast.error(err.response?.data?.msg || 'فشل إرسال الطلب');
                }
              }}>
                    ...
                    return (
                      <label key={type} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: viewLocked ? 'not-allowed' : 'pointer', fontSize: 13, padding: '4px 0', opacity: viewLocked ? 0.5 : 1 }}>
                        ...
                        <span style={{ fontWeight: requestActionTypes.has(type) ? 600 : 400 }}>
                          ...
                          {type === 'upload' ? 'طلب رفع مستند — طلب رفع مستند من المتدرب (بدون قالب)' : ''}
                        </span>
                      </label>
                    );
                  })}
                  ...
                  return (
                    <button key={type} onClick={async () => {
                      try {
                        ...
                        fetchDocsLifecycle();
                      } catch (err: any) { toast.error(err.response?.data?.msg || 'فشلت الإضافة من الخزنة'); }
                    }} style={{
